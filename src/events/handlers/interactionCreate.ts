...
let event: Event<"interactionCreate"> = {
  ...
  run: async (client, interaction) => {
    ...
    if (interaction.isChatInputCommand())
      CommandType = CommandTypes.SlashCommand;
    else if (interaction.isUserContextMenuCommand())
      CommandType = CommandTypes.UserContextMenuCommand;
    else if (interaction.isMessageContextMenuCommand())
      CommandType = CommandTypes.MessageContextMenuCommand;
    else return false;

    if (!CommandType) return false;

    let commandsCollection = client.commands.get(CommandType);

    if (!commandsCollection) return false;

    let command = commandsCollection.get(interaction.commandName) as
      | SlashCommand
      | ContextMenuCommand;
    ...
    try {
      if (
        command.type === CommandTypes.SlashCommand &&
        interaction.isChatInputCommand()
      )
        await command.execute(client, interaction);
      else if (
        command.type === CommandTypes.MessageContextMenuCommand &&
